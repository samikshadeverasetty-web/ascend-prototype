...
import {
  ...
  Plus, Lock, RotateCcw, Mail, Eye, EyeOff
} from "lucide-react";
...
function Login({ onLogin }) {
  ...
  function handleSubmit(e) {
    ...
    // Prototype-only authentication: no real password is stored here.
    // Replace this with Firebase/Supabase/Auth0 or your own backend for production.
    sessionStorage.setItem("ascend_session", "active");
    if (remember) localStorage.setItem("ascend_remember", "true");
    onLogin(cleanEmail);
  }
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: T.bg, color: T.text, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ position: "fixed", inset: 0, pointerEvents: "none", background: `radial-gradient(circle at 50% 20%, ${T.violetDim} 0, transparent 38%), radial-gradient(circle at 80% 80%, ${T.tealDim} 0, transparent 32%)` }} />
        <div className="fade-up" style={{ width: "100%", maxWidth: 440, position: "relative" }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            <div className="font-display" style={{ display: "inline-flex", alignItems: "center", gap: 10, fontSize: 28, fontWeight: 800 }}>
              <div style={{ width: 42, height: 42, borderRadius: 12, background: `linear-gradient(135deg, ${T.teal}, ${T.violet})`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Sparkles size={22} color="#08110F" />
              </div>
              Ascend
            </div>
            <div style={{ color: T.dim, marginTop: 10, fontSize: 14 }}>Smart learning that adapts to you.</div>
          </div>

          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 22, padding: 28, boxShadow: "0 24px 70px rgba(0,0,0,.35)" }}>
            <div style={{ marginBottom: 22 }}>
              <h1 className="font-display" style={{ margin: 0, fontSize: 24 }}>Welcome back</h1>
              <p style={{ margin: "7px 0 0", color: T.dim, fontSize: 13 }}>Sign in to continue your learning journey.</p>
            </div>

            <form onSubmit={handleSubmit}>
              <label style={{ display: "block", color: T.dim, fontSize: 12, fontWeight: 600, marginBottom: 7 }}>EMAIL</label>
              <div style={{ position: "relative", marginBottom: 16 }}>
                <Mail size={17} color={T.faint} style={{ position: "absolute", left: 13, top: 14 }} />
                <input value={email} onChange={e => setEmail(e.target.value)} type="email" autoComplete="email" placeholder="you@example.com" style={{ width: "100%", padding: "12px 13px 12px 40px", borderRadius: 11, border: `1px solid ${T.borderLight}`, background: T.surface2, color: T.text, outline: "none", fontSize: 14 }} />
              </div>

              <label style={{ display: "block", color: T.dim, fontSize: 12, fontWeight: 600, marginBottom: 7 }}>PASSWORD</label>
              <div style={{ position: "relative", marginBottom: 12 }}>
                <Lock size={17} color={T.faint} style={{ position: "absolute", left: 13, top: 14 }} />
                <input value={password} onChange={e => setPassword(e.target.value)} type={showPassword ? "text" : "password"} autoComplete="current-password" placeholder="Enter your password" style={{ width: "100%", padding: "12px 44px 12px 40px", borderRadius: 11, border: `1px solid ${T.borderLight}`, background: T.surface2, color: T.text, outline: "none", fontSize: 14 }} />
                <button type="button" onClick={() => setShowPassword(v => !v)} aria-label={showPassword ? "Hide password" : "Show password"} style={{ position: "absolute", right: 10, top: 8, width: 34, height: 34, border: 0, background: "transparent", color: T.dim, cursor: "pointer" }}>
                  {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>

              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 8, color: T.dim, fontSize: 12, cursor: "pointer" }}>
                  <input type="checkbox" checked={remember} onChange={e => setRemember(e.target.checked)} /> Remember me
                </label>
                <span style={{ color: T.violet, fontSize: 12, fontWeight: 600 }}>Forgot password?</span>
              </div>

              {error && <div role="alert" style={{ background: T.coralDim, border: `1px solid ${T.coral}`, color: T.text, borderRadius: 10, padding: "10px 12px", fontSize: 12, marginBottom: 14 }}>{error}</div>}

              <button type="submit" style={{ width: "100%", border: 0, borderRadius: 11, padding: "13px 16px", background: `linear-gradient(135deg, ${T.teal}, #42B8D6)`, color: "#06100E", fontWeight: 800, fontSize: 14, cursor: "pointer" }}>
                Sign in <ArrowRight size={16} style={{ verticalAlign: "middle", marginLeft: 5 }} />
              </button>
            </form>

            <div style={{ display: "flex", alignItems: "center", gap: 9, marginTop: 20, paddingTop: 18, borderTop: `1px solid ${T.border}`, color: T.dim, fontSize: 11 }}>
              <ShieldCheck size={16} color={T.teal} />
              Secure session • Passwords are not stored in this prototype
            </div>
          </div>
          <div style={{ textAlign: "center", marginTop: 16, color: T.faint, fontSize: 11 }}>Ascend • Student Learning Platform</div>
        </div>
      </div>
    </>
  );
...
export default function App() {
  ...
  const [authenticated, setAuthenticated] = useState(() => sessionStorage.getItem("ascend_session") === "active");
  ...
  if (!authenticated) {
    return <Login onLogin={(email) => { setUserEmail(email); setAuthenticated(true); setStarted(true); }} />;
  }
